...
def certificados_evento(request, id):
    ...
    if request.method == "GET":
        ...
        return render(
            ...
            {"evento": evento, "qtd_certificados": qtd_certificados, "evento": evento},
        )
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        draw.text(
            (230, 651), f"{participante.username}", font=fonte_nome, fill=(0, 0, 0)
        )
        draw.text((761, 782), f"{evento.nome}", font=fonte_info, fill=(0, 0, 0))
        draw.text(
            (816, 849), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0, 0, 0)
        )
